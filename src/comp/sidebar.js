...
export default function Sidebar(props) {
    ...
    return(
        <div className="flex fixed bg-white  h-screen ">
          <ul className="flex flex-col gap-3 w-full mt-10">
            <Link href={"/"}><li className={`${props.home} px-3 ml-2 py-2 rounded-l-3xl hover:bg-slate-200`}>{home}</li></Link>
            <Link href={"/user"}><li className={`${props.user} px-3 ml-2 py-2 rounded-l-3xl hover:bg-slate-200`}>{user}</li></Link>
            <Link href={"/input"}><li className={`${props.input} px-3 ml-2 py-2 rounded-l-3xl hover:bg-slate-200`}>{input}</li></Link>
            <Link href={"/target"}><li className={`${props.target} px-3 ml-2 py-2 rounded-l-3xl hover:bg-slate-200`}>{target}</li></Link>
          </ul>
        </div>
    )
}